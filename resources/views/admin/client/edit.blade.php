...
@section('content')
    ...
    <main class="w-full flex-grow p-6 bg-gray-50">
        ...
        @role('admin')
            <form action="{{ route('clients.update', $client) }}" method="post" class="w-full max-w-2xl mx-auto">
                @csrf
                @method('PUT')
                ...
                <div class="flex items-center justify-between mb-4">
                    <select name="active" id="role"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                        <option value="0" {{ $client->active ? 'selected' : '' }}>Inactive</option>
                        <option value="1" {{ $client->active ? 'selected' : '' }}>Active</option>
                    </select>
                </div>
